...
import { Heart, Mail, Lock, User, Eye, EyeOff, ArrowRight, Shield, Check, AlertCircle, CheckCircle } from 'lucide-react'
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-white p-8">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md w-full text-center">
          ...
          <p className="text-slate-500 mb-6">
            We sent a confirmation link to <span className="font-medium text-slate-700">{formData.email}</span>.
            Click the link to activate your account.
          </p>
          <Link href="/login" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 transition-colors">
            Back to sign in
          </Link>
        </motion.div>
      </div>
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-teal-600 via-emerald-600 to-indigo-700 items-center justify-center p-12">
        ...
        >
          ...
          <h1 className="text-4xl font-bold leading-tight mb-4">Start your health journey today</h1>
          <p className="text-emerald-100 text-lg leading-relaxed mb-10">
            Join thousands who have taken control of their medical data. Free forever, no credit card required.
          </p>
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.1 }} className="flex items-center gap-3 mb-4">
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-white">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
          ...
          <p className="text-slate-500 text-sm mb-8">Start managing your health records for free</p>
          ...
          <form onSubmit={handleSignup} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} placeholder="Sarah Johnson" className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all" required />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={formData.email} onChange={e => setFormData(p => ({ ...p, email: e.target.value }))} placeholder="you@example.com" className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all" required />
              </div>
            </div>
              ...
              {formData.password.length > 0 && (
                <div className="mt-2">
                  <div className="flex gap-1 mb-1">
                    {[1, 2, 3].map(i => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= passwordStrength ? strengthColors[passwordStrength] : 'bg-slate-200'}`} />
                    ))}
                  </div>
            ...
            <div className="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 border border-slate-100">
              ...
              <p className="text-xs text-slate-600">Your health data is encrypted and protected. We never sell or share your information.</p>
            </div>

            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} type="submit" disabled={isLoading} className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm transition-colors disabled:opacity-70 shadow-lg shadow-indigo-500/25">
              {isLoading ? <div className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <>Create free account <ArrowRight className="h-4 w-4" /></>}
            </motion.button>
          </form>
